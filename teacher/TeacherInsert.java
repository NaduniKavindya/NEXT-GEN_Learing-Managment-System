...
public class TeacherInsert extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		isTrue = TeacherDBUtil.insertteacher(tid,name,nic,gender,subject,address,email,phone,experience,eduqualifications,username,password);
		
		if(isTrue == true) {
			RequestDispatcher dis = request.getRequestDispatcher("Tlogin.jsp");
			dis.forward(request, response);
		} else {
			RequestDispatcher dis2 = request.getRequestDispatcher("Tlogin.jsp");
