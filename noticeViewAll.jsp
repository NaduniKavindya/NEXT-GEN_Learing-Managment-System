...
<br><br><br><br><br><br>
<table  id="tblCustomers" class="container" width="1000px" align="center" >
...
if(request.getAttribute("allList")!=null)
{
	...
	while(itr.hasNext())
	{
		...
		count++;
		ArrayList notList = (ArrayList) itr.next();
		%>
...
<td><%=notList.get(4)%></td>
<td><%=notList.get(5)%></td>
<td><form action = "noticeUpdateHelper" method = "post"><button type="submit" name = "submit" value = "<%=notList.get(0)%>">Update</button></form></td>
<td><form action = "noticeDelete" method = "post" onsubmit=return(geek())><button type= "submit" name = "submit" value = "<%=notList.get(0)%>">Delete</button></form></td>
</tr>

<script>
        function geek() {
            var x = confirm("Are you sure to delete this Notice?");
            if (x)
              return true;
            else
              return false;
...
if(count==0)
{
	%>
<tr>
<td colspan=8 align="center" style="background-color:eeffee"><b>No Notices available...</b></td>
